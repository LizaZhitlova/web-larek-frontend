...
Поле класса:
items: Set<string> - поле items хранит коллекцию уникальных индентификаторов товара(id).
Методы класса:

add(id: string): void - Добавляет товар в корзину. Принимает в качестве аргумента идентификатор товара.

Делет проверку наличия товара по id.
Если товар уже есть — метод ничего не делает (предотвращает повторное добавление).
Если же товара нет, то добавляет его.
После изменения в корзине вызывается событие событие 'basket:change'.

remove(id: string): void - Удаляет товар из корзины.Принимает в качестве аргумента идентификатор товара.

Проверяет наличие товара в корзине:если товар с указанным id отсутствует в множестве (Set), метод завершает выполнение.
Если товар сеть в корзине, то выполняет удаление.
После изменения в корзине вызывается событие событие 'basket:change'.
...
hasProduct(id: string): boolean -метод  проверяет, присутствует ли товар с указанным id в корзине и возвращает булевое значение.
...
render(product: IProduct,isblocked:boolean) - отвечает за обновление эелемнта карточки товара и блокировку кнопки добавленния товара в корзину. В качестве агрумента принимает объект товара типа IProduct и булевое значение isblocked.
...
В зависимисто от того какое булевое значение передаётся в аргумет - активирует или деактивирует кнопку добавления товара в корзину.

## Класс OrderView

- отвечает за отображение и обработку первого шага оформления заказа(выбор способа оплаты,ввод адреса доставки).
  Клксс отслеживает состояние этих полей, валидирует их и активирует кнопку "Далее", если данные корректны. Наследуется от абстрактного класса Form.
...
protected onSubmit() -отвечает за активацию кнопки "Далее"
